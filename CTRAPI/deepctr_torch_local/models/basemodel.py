...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
            validation_data=None, shuffle=True, callbacks=None, recorder=None, logger=None):
        """
        ...
        :param callbacks: List of `deepctr_torch_local.callbacks.Callback` instances. List of callbacks to apply during training and validation (if ). See [callbacks](https://tensorflow.google.cn/api_docs/python/tf/keras/callbacks). Now available: `EarlyStopping` , `ModelCheckpoint`
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            try:
                ...
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    for _i, (x_train, y_train) in t:
                        ...
                        pred_list = list(y_pred.cpu().detach().numpy().reshape(-1))
                        for i in pred_list:
                            if math.isnan(i):
                                raise RuntimeError('Found nan in pred list')
            ...
            if do_validation:
                ...
                for name, result in eval_result.items():
                    if name == 'auc' and recorder is not None:
                        go_on = recorder.update(model, result, epoch)
            ...
            if verbose > 0:
                ...
                if go_on is False:
                    logger.info(f'Early-stop strategy stop the training at epoch-{epoch}')
                    break
