...
class InterestExtractor(nn.Module):
    ...
    def forward(self, keys, keys_length, neg_keys=None):
        ...
        packed_keys = pack_padded_sequence(masked_keys, lengths=masked_keys_length.to('cpu'), batch_first=True,
...
class InterestEvolving(nn.Module):
    ...
    def forward(self, query, keys, keys_length, mask=None):
        ...
        if self.gru_type == 'GRU':
            packed_keys = pack_padded_sequence(keys, lengths=keys_length.to('cpu'), batch_first=True, enforce_sorted=False)
            ...
            packed_interests = pack_padded_sequence(interests, lengths=keys_length.to('cpu'), batch_first=True,
                                                    enforce_sorted=False)
            ...
            packed_interests = pack_padded_sequence(keys, lengths=keys_length.to('cpu'), batch_first=True,
                                                    enforce_sorted=False)
            packed_scores = pack_padded_sequence(att_scores, lengths=keys_length.to('cpu'), batch_first=True,
                                                 enforce_sorted=False)
