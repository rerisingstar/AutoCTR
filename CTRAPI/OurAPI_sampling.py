...
class CTR(nn.Module):
    ...
    def CTRData(self, full_data_name, searching_flag=False, ratio=0.2, random_state=2020, sample_ratio=1.0):
        ...
        if data_name == 'Amazon':
            ...
            feature_names['vallen'] = ['hist_mid', 'hist_cat']
        elif data_name in ['MovieLens1m', 'MovieLens10m', 'MovieLens20m']:
            feature_names['sparse'] = ["uid", "mid", 'cat']
            feature_names['dense'] = []
            feature_names['vallen'] = ['hist_mid', 'hist_cat']
        ...
        for index in range(2):
            splited = [
                train_test_split(part_data[feat], test_size=0.2, random_state=random_state*index)
        ...
        for index in range(3):
            splited_biased = [
                train_test_split(part_data[feat][data_num_split[index]:data_num_split[index+1]], test_size=0.5, random_state=random_state*index)
                for feat in part_data.keys()
            ]
            splited = [
                train_test_split(part_data[feat], test_size=0.05, random_state=random_state * index)
                for feat in part_data.keys()
            ]
            model_input = {}
            for i, name in enumerate(feature_names_list):
