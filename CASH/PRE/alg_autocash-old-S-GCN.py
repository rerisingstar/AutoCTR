...
class AutoCASH(Random):
    ...
    def train_new_dataset(self, NewInfo, dataset, epochs=1, batch_size=32, val=False):
        ...
        for epoch in range(epochs):
            ...
            for _, (di, si, y) in enumerate(train_loader):
                ...
                solution_emb, _ = self.Meta.get_solution_embedding(self.gcn_flag, si, self.device)
    ...
    def get_random_solution(self):
        ...
        if 0 in model_visited_num_list :
            real_model_visited_num_list = []
            model_name = None
            for i in range(len(model_names)):
                if self.model_visited_num[model_names[i]] == 0:
                    real_model_visited_num_list.append(model_names[i])
                    if model_names[i] in good_models:
                        model_name = model_names[i]
                        break
            if model_name == None:
                random.shuffle(real_model_visited_num_list)
                model_name = random.sample(real_model_visited_num_list,1)[0]
        elif 1 in model_visited_num_list or 2 in model_visited_num_list:
            real_model_visited_num_list = []
            model_name = None
            for i in range(len(model_names)):
                if self.model_visited_num[model_names[i]] < 3:
                    real_model_visited_num_list.append(model_names[i])
                    if model_names[i] in good_models:
                        model_name = model_names[i]
                        break
