...
class MetaPredictor(nn.Module):
    ...
    def evaluate(self, test_loader):
        with torch.no_grad():
            ...
            for _, (di, si, y) in enumerate(test_loader):
                ...
                solution_emb, _ = self.get_solution_embedding(False, si, self.device, sample_num=100)
                y = y.to(self.device).to(torch.float32)
                y_pred = self(di, solution_emb)
        ...
        if gcn_flag:
            with torch.no_grad():
                ...
                for i in range(len(target_nodes)):
                    ...
                    self.Semb.weight[target_node] = new_solution_emb[i]
                #for i in range(len(added_solution_info)):
    ...
    def construct(self, epochs=10, sample_num=100):
        ...
        for epoch in range(epochs):
            ...
            test_loss, test_loss2 = self.evaluate(self.test_loader)
