...
class AutoCASH(Random):
    def __init__(self, args):
        ...
        self.use_attn = False if args.use_attn=='False' else True 
        if self.use_attn:
            attn_name = ""
        else:
            attn_name = "_noattn"
        if args.retain_metapredictorepochs > 0:
            meta_predicter = MetaPredictor(device=device_name, meta_path=base_dir+'/autocash/meta_full.json',
                                           use_attn=self.use_attn, meta_ratio=self.meta_ratio,
        ...
        self.logger.info(f'--- self.use_attn: {self.use_attn}')
        self.logger.info(f'--- self.use_attn==False: {self.use_attn==False}')
    ...
    def _init_center_info(self, center_ids, dataset_id):
        ...
        with torch.no_grad():
            center_embs, _ = self.Meta.get_solution_embedding(self.use_attn, center_ids, self.device)
    ...
    def k_means(self, dataset, st_batch_size=512):
        ...
        for ks_time in range(ks_times):
            for _, (s, t) in enumerate(batch_begin_ends):  # meta_record
                ...
                with torch.no_grad():
                    st_embs, _ = self.Meta.get_solution_embedding(self.use_attn, st_batch, self.device)
                # tensor(B, E)
            ...
            for c_id, cluster in enumerate(clusters):
                ...
                for idx in range((len(cluster_solutions) - 1) // batch_size + 1):
                    ...
                    with torch.no_grad():
                        cluster_embs_tmp, _ = self.Meta.get_solution_embedding(self.use_attn, cluster_solutions_tmp, self.device)
    ...
    def get_best_sl_incluster(self, cluster, dataset_id):
        ...
        # tensor(C, 1)
        #solution_embs = self.Meta.Semb(solution_ids).squeeze()
        solution_ids = torch.tensor(random.sample(list(cluster), 10000)).view(-1, 1).to(self.device)
        ...
        for idx in range((len(solution_ids) - 1) // batch_size + 1):
            ...
            with torch.no_grad():
                solution_embs, _ = self.Meta.get_solution_embedding(self.use_attn, solution_ids_tmp.squeeze(), self.device)
                # tensor(C, E)
    ...
    def get_sample_sl_incluster(self, cluster, dataset_id):
        ...
        for idx in range((len(solution_ids) - 1) // batch_size + 1):
            solution_ids_tmp = solution_ids[idx * batch_size: min(idx * batch_size + batch_size, len(solution_ids))]
            with torch.no_grad():
                solution_embs, _ = self.Meta.get_solution_embedding(self.use_attn, solution_ids_tmp, self.device)
    ...
    def fine_tune(self, NewInfo, dataset, epochs=1, batch_size=32):
        ...
        for epoch in range(epochs):
            ...
            for _, (di, si, y) in enumerate(train_loader):
                ...
                loss.backward()
                optimizer.step()
                #self.Meta.update_embeddings(self.use_attn, si, self.device, sample_num=self.gcn_samplenum)
            totalloss /= len(train_loader)
            self.logger.info(f"--- epoch-{epoch}, loss={totalloss}")
