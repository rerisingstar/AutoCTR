...
class MetaPredictor(nn.Module):
    ...
    def construct_input(self, x, device, solution_sample_num=200):
        solution_q = x
        solution_k = []
        model_type_q = []
        model_type_k = []
        ...
        return solution_q, None, model_type_q, model_type_k, model_component_k
    ...
    def get_solution_embedding(self, use_attn, target_nodes, device, solution_sample_num=200):
        '''
        :param x: (B, 1)
        :return: predict_auc: (B, 2)
        '''
        if use_attn:
            solution_q, solution_k, model_type_q, model_type_k, model_component_k = self.construct_input(target_nodes, device, solution_sample_num)
            ...
            try:
                component_emb, _ = self.multihead_component(solution_q.unsqueeze(1), model_type_emb, model_type_emb)
            except:
                component_emb, _ = self.multihead_component(solution_q, model_type_emb, model_type_emb)
